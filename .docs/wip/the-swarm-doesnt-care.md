...
I've been sitting on an answer to that for a few months now, mostly because I was too busy building it to write about it.
...
Which means a Python worker that loads a model once and serves inference requests all day is architecturally identical to a Maya worker that loads a scene once and processes rock scans all day. Same pull loop. Same job queue. Same priority system. Same monitoring. Here's the Maya worker loop from Part 3:
...
Same shape. Poll, check for work, execute, report, loop. One of them happens to have Maya loaded in memory. The other has a diffusion model. The swarm doesn't know the difference and doesn't need to.
...
I keep seeing the industry conversation frame this as a binary: either AI replaces the pipeline, or it doesn't touch it. I don't think that's the real fork. The real fork is whether AI shows up as a one-off script somebody wires up badly, or as another bounded job type inside infrastructure that already knows how to schedule, validate, retry, and monitor work.

I already have the second option sitting on my machine.
...
*Have thoughts on AI workers riding existing job infrastructure instead of getting their own pipeline? Let me know in the comments or reach out on [Twitter](https://twitter.com/hogjonny) or [LinkedIn](https://www.linkedin.com/in/hogjonny).*
